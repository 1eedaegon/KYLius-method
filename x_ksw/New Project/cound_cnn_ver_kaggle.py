...
#placeholder
X = tf.placeholder(tf.float32, [None, 40, 173,1], name="X")
...
W1 = tf.get_variable("W1", shape=[4, 10, 1, 32],initializer=tf.contrib.layers.xavier_initializer())
L1 = tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME')
L1 = tf.layers.batch_normalization(L1)
L1 = tf.nn.relu(L1)
#L1 = tf.nn.elu(L1)
#L1 = tf.nn.leaky_relu(L1,0.1)
#L1 = tf.nn.max_pool(L1, ksize=[1, 2, 10, 1],strides=[1, 2, 10, 1], padding='SAME') 
L1 = tf.nn.max_pool(L1, ksize=[1, 2, 2, 1],strides=[1, 2, 2, 1], padding='VALID') 
L1 = tf.nn.dropout(L1, p_keep_conv)

# L2 Input shape=(?,14,58,32)
W2 = tf.get_variable("W2", shape=[4, 10, 32, 32],initializer=tf.contrib.layers.xavier_initializer())
...
W3 = tf.get_variable("W3", shape=[4, 10, 32, 32],initializer=tf.contrib.layers.xavier_initializer())
...
L3_flat= tf.reshape(L3, shape=[-1, 5*21*32])

# Final FC 2*3*128 inputs -> 41 outputs
W4 = tf.get_variable("W4", shape=[5*21*32, 64],initializer=tf.contrib.layers.xavier_initializer())
L4 = tf.nn.relu(tf.matmul(L3_flat, W4))
#L4 = tf.nn.elu(tf.matmul(L3_flat, W4))
#L4 = tf.nn.leaky_relu(tf.matmul(L3_flat, W4),0.1)
L4 = tf.nn.dropout(L4, p_keep_hidden)
W_o = tf.get_variable("W_o", shape=[64,41],initializer=tf.contrib.layers.xavier_initializer())
...
accuracy: 60~61%
910) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.22까지 나옴) 정확도는 그대로임
lr=0.00005, epoch = 500    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (2, 10, 32), (3, 3, 64), (3, 3, 128)
