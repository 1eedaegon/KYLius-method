...
def extract_feature(file_name):
    ...
    mfccs = np.mean(librosa.feature.mfcc(y=X, sr=sample_rate, n_mfcc=784,hop_length=int(sample_rate*0.01),n_fft=int(sample_rate*0.02),htk=True).T,axis=0) #오디오 신호를 mfcc로 바꿈
...
df.to_csv("/home/itwill03/sound/df_train.csv", sep=',',index=False,mode='w')
...
train = pd.read_csv('/home/itwill03/sound/df_train.csv')

#훈련세트, validation세트 나누기
from sklearn.model_selection import train_test_split
train_set, validate_set = train_test_split(train, test_size = 0.3)
trainData = train_set.values[:,1:-2]
validateData = validate_set.values[:,1:-2]
