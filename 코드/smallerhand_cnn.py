...
# L3_2
W3_2 = tf.Variable(tf.random_normal([3, 3, 64, 128], stddev=0.01))
L3_2 = tf.nn.conv2d(L2_2, W3_2, strides=[1, 1, 1, 1], padding='SAME')
L3_2 = tf.nn.relu(L3_2)
L3_2 = tf.nn.max_pool(L3_2, ksize=[1, 2, 2, 1],
                    strides=[1, 2, 2, 1], padding='SAME')
L3_2 = tf.nn.dropout(L3_2, keep_prob=keep_prob)
L3_2_flat = tf.reshape(L3_2, [-1, 4 * 4 * 128])

# Final FC 7x7x64 inputs -> 10 outputs
W4_1 = tf.get_variable("W4_1", shape=[4 * 4 * 128, 10],
                     initializer=tf.contrib.layers.xavier_initializer())
W4_2 = tf.get_variable("W4_2", shape=[4 * 4 * 128, 10],
                     initializer=tf.contrib.layers.xavier_initializer())
b = tf.Variable(tf.random_normal([10]))
logits = tf.matmul(L3_flat, W4_1) + tf.matmul(L3_2_flat, W4_2) + b
