...
p1 = tf.layers.max_pooling2d(inputs=n1, pool_size=[3, 3], strides=2) 
p1 = tf.nn.dropout(p1, p_keep_conv)

# img shape = (?, 64, 64, 32)
c2 = tf.layers.conv2d(tf.reshape(p1, [-1, 63, 63, 32]), 64, kernel_size=[5, 3], strides=(1, 1), 
...
# img shape = (?, 32, 32, 64)
c3 = tf.layers.conv2d(tf.reshape(p2, [-1, 31, 31, 64]), 128, kernel_size=[5, 3], strides=(1, 1), 
...
L4_flat = tf.reshape(p3, shape=[-1, 15*15*128]) 
W1 = tf.get_variable("W1", shape=[15*15*128, 640], initializer=tf.contrib.layers.xavier_initializer())
