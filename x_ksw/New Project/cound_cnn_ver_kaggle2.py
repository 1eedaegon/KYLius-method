...
class Config(object):
    def __init__(self,
        ...
        self.audio_length = self.sampling_rate * self.audio_duration*2+4
        if self.use_mfcc:
            self.dim = (self.n_mfcc, 6 + int(np.floor(self.audio_length/512)), 1)
        else:
...
config.dim[0]
config.dim[1]
#X = np.empty(shape=(9473, 40, 350, 1)) #  엔트리를 초기화 하지 않고 값을 반환
...
trainData.shape # (6631, 40, 350)
testData.shape # (2842, 40, 350)
...
#placeholder
X = tf.placeholder(tf.float32, [None, 40, 350,1], name="X")
#X_sound = tf.reshape(X, [-1, 40, 173, 1])          # 40*350*1 (frequency, time, amplitude)
...
W4 = tf.get_variable("W4", shape=[2*2*128, 410],initializer=tf.contrib.layers.xavier_initializer())
#L4 = tf.nn.relu(tf.matmul(L3_flat, W4))
#L4 = tf.nn.elu(tf.matmul(L3_flat, W4))
L4 = tf.nn.leaky_relu(tf.matmul(L3_flat, W4),0.1)
L4 = tf.nn.dropout(L4, p_keep_hidden)
W_o = tf.get_variable("W_o", shape=[410,41],initializer=tf.contrib.layers.xavier_initializer())
...
for epoch in range(training_epochs):
    ...
    if epoch % steps_for_validate == steps_for_validate-1:
        ...
        print('Accuracy:', sess.run(accuracy, feed_dict={
                X: X_test, Y: testLabel.reshape(-1, 1), p_keep_conv: 1, p_keep_hidden: 1}))
...
accuracy: 61~63%
5) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.0016까지 나옴) 정확도는 그대로임
lr=0.0002, epoch = 300    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (3, 13, 32), (3, 3, 64), (3, 3, 128)
FC : (5*22*128, 625), (625, 41)
max_pool : (2,2), (2,2), (2,2)
accuracy: 53~56%
6) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.16까지 나옴) 정확도는 그대로임
lr=0.00008, epoch = 300    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (2, 10, 32), (3, 3, 64), (3, 3, 128)
FC : (3*2*128, 625), (625, 41)
max_pool : (2,10), (3,3), (3,3)
accuracy: 65~67%
7) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.13까지 나옴) 정확도는 그대로임
lr=0.00005, epoch = 500    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (2, 10, 32), (3, 3, 64), (3, 3, 128)
FC : (3*2*128, 625), (625, 41)
max_pool : (2,10), (3,3), (3,3)
accuracy: 65~66%
8) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.008까지 나옴) 정확도는 그대로임
lr=0.00005, epoch = 400    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (3, 3, 32), (3, 3, 64), (3, 3, 128)
FC : (5*5*128, 1000), (1000, 41)
max_pool : (2,10), (2,2), (2,2)
accuracy: 60~61%
9) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.22까지 나옴) 정확도는 그대로임
lr=0.00005, epoch = 500    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (2, 10, 32), (3, 3, 64), (3, 3, 128)
FC : (5*1*128, 650), (650, 41)
max_pool : (2,10), (2,10), (3,3)
accuracy: 60~61%
10) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.22까지 나옴) 정확도는 그대로임
lr=0.00005, epoch = 500    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (2, 10, 32), (3, 3, 64), (3, 3, 128)
FC : (10*10*128, 650), (650, 41)
max_pool : (2,9), (2,2), (1,1)
accuracy: 59~60%
11) 
    cost = 0.024251443
    Accuracy: 0.64% ~ 0.65%
12) con2d layer * 3 + FC # 코스트는 계속해서 내려가나 (0.024까지 나옴) 정확도는 그대로임
lr=0.0002, epoch = 700    
p_keep_conv, p_keep_hidden = 0.8, 0.7
win : (4, 10, 32), (3, 3, 64), (3, 3, 128)
FC : (5*6*128, 650), (650, 41)
max_pool : (4,10), (2,2), (2,2)
accuracy: 61~62%
"""
