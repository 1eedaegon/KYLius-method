...
class Config(object):
    def __init__(self,
        ...
        self.audio_length = self.sampling_rate * self.audio_duration*2.61
        if self.use_mfcc:
...
#placeholder
X = tf.placeholder(tf.float32, [None, 40, 450,1], name="X") # 40*350*1 (frequency, time, amplitude)
...
# L1 SoundIn shape=(?, 40, 450, 1)
W1 = tf.get_variable("W1", shape=[2, 23, 1, 32],initializer=tf.contrib.layers.xavier_initializer())
L1 = tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME')
L1 = tf.layers.batch_normalization(L1)
#L1 = tf.nn.relu(L1)
#L1 = tf.nn.elu(L1)
L1 = tf.nn.leaky_relu(L1,0.1)
L1 = tf.nn.max_pool(L1, ksize=[1, 2, 23, 1],strides=[1, 2, 23, 1], padding='SAME') 
...
L3_flat= tf.reshape(L3, shape=[-1, 10*10*128])

# Final FC 2*2*128 inputs -> 41 outputs
W4 = tf.get_variable("W4", shape=[10*10*128, 1250],initializer=tf.contrib.layers.xavier_initializer())
#L4 = tf.nn.relu(tf.matmul(L3_flat, W4))
#L4 = tf.nn.elu(tf.matmul(L3_flat, W4))
L4 = tf.nn.leaky_relu(tf.matmul(L3_flat, W4),0.1)
L4 = tf.nn.dropout(L4, p_keep_hidden)
W_o = tf.get_variable("W_o", shape=[1250,41],initializer=tf.contrib.layers.xavier_initializer())
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        feed_dict = {X: batch_xs, Y: batch_ys, p_keep_conv: 0.7, p_keep_hidden: 0.5}
