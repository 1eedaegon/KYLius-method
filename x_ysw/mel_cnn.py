...
#normalizaion
#train_data = librosa.util.normalize(trainData, axis = 1, norm = 0) #axix = ?, norm = ?>0>?
...
p1 = tf.layers.max_pooling2d(inputs=n1, pool_size=[12, 4], strides=4) 
p1 = tf.nn.dropout(p1, p_keep_conv)

# img shape = (?, 64, 64, 32)
c2 = tf.layers.conv2d(tf.reshape(p1, [-1, 30, 32, 32]), 64, kernel_size=[5, 3], strides=(1, 1), 
                      padding='same', activation=tf.nn.relu, name="c2")
n2 = tf.layers.batch_normalization(c2) 
p2 = tf.layers.max_pooling2d(inputs=n2, pool_size=[12, 4], strides=2) #shape = [?, 1, 48, 100]
p2 = tf.nn.dropout(p2, p_keep_conv)

# img shape = (?, 32, 32, 64)
c3 = tf.layers.conv2d(tf.reshape(p2, [-1, 10, 15, 64]), 128, kernel_size=[5, 3], strides=(1, 1), 
...
L4_flat = tf.reshape(p3, shape=[-1, 5*7*128]) 
W1 = tf.get_variable("W1", shape=[5*7*128, 420], initializer=tf.contrib.layers.xavier_initializer())
L5 = tf.nn.relu(tf.matmul(L4_flat, W1))
n5 = tf.layers.batch_normalization(L5) 
L5 = tf.nn.dropout(n5, p_keep_hidden)

W2 = tf.get_variable("W2", shape=[420,41],initializer=tf.contrib.layers.xavier_initializer())
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        feed_dict = {X: batch_xs, Y: batch_ys, p_keep_conv: .8, p_keep_hidden: .7}
